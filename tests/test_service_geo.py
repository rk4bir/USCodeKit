...
from uscodekit.services.geo import GeoService, get_cached_database, get_database
...
#     cipher_suite = Fernet(mock_key)
#     mock_data = [{"npa": "907", "nxx": "200", "npanxx": "907200", "city": "Valdez", "state": "Alaska", "stateISO": "AK", "country": "United States", "countryISO": "US", "zipCode": "99686", "gmtOffset": "-9", "gmtOffsetDST": "-8", "dstObserved": "1", "longitude": "-146.3572", "timezone": {"name": "AKST", "offset": "UTC-9"}, "location": {"latitude": 61.1381, "longitude": -146.3572}}]
#     encrypted_data = cipher_suite.encrypt(json.dumps(mock_data).encode("utf-8"))

#     with patch("builtins.open", mock_open(read_data=mock_key)) as mock_key_file:
#         with patch("gzip.open", mock_open(read_data=encrypted_data)) as mock_gzip_file:
#             with patch("builtins.open", mock_open(read_data=encrypted_data)) as mock_decompressed_file:
#                 result = get_database()
#                 assert result == mock_data
